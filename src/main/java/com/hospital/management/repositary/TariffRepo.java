...
public interface TariffRepo extends JpaRepository<Tariff, Integer> {
    @Query(value = "SELECT * FROM hms.tariff t WHERE t.del_tariff = 0", nativeQuery = true)
    List<Tariff> findAllTariffs();

    @Query(value = "UPDATE hms.tariff SET del_tariff = 1 WHERE tariff_id = :tariffId", nativeQuery = true)
